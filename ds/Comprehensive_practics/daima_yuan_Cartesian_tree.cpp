...
using namespace std;
int t,a[maxn],n,root,ans[maxn],cnt=1;
struct info
{
    int val; // 代表这个点的数值
...
void dfs(int x){
    ans[x]=cnt;cnt++;
    if(seg[x].lef) dfs(seg[x].lef);
    if(seg[x].rig) dfs(seg[x].rig);
    return ;
}
void slove(){
    cin>>n;for(int i=1;i<=n;i++){
        cin>>a[i];if(a[i]==1) root=i;
    }
    build();
    dfs(root);for(int i=1;i<=n;i++){
        cout<<ans[i]<<" ";
    }
	return;
}
